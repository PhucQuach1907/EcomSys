...
def login(request):
    ...
    response = Response({'token': token.key, 'user_id': user.pk, 'email': user.email}, status=status.HTTP_200_OK)
    response.set_cookie('token', token.key, httponly=True, max_age=3600)
    return response

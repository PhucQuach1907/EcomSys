...
function generateRandomPaymentCode(length) {
...
function takeShipmentInfo() {
    ...
    submitButton.addEventListener('click', function () {
        ...
        paymentCode = generateRandomPaymentCode(12);
        my_bank.DESCRIPTION = my_bank.DESCRIPTION + ' ' + paymentCode;
...
async function checkPaid(name, mobileNumber, address) {
    try {
        ...
        if (lastPaid["Giá trị"] >= my_bank.AMOUNT && lastPaid["Mô tả"].includes(paymentCode)) {
            console.log("success!");
            initOrder()
                .then(response => {
                    return response.json()
...
function initOrder() {
    return fetch('http://127.0.0.1:8087/order/api/init-order/', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function handleUpdatePaymentStatusResponse(response, data, name, mobileNumber, address) {
    if (response.ok) {
        ...
        fetch(`http://127.0.0.1:8087/order/api/init-order/${data.order_id}/`)
            .then(response => {
                return response.json()
                    .then(data => {
